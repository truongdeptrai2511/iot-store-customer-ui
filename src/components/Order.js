...
const Order = ({ order, onRemove }) => {
  ...
  const handleRemoveClick = (Id, OrderId) => {
    ...
    const updatedOrder = currentOrders.filter((order) => order.productId !== Id);
    setCookie('orders', JSON.stringify(updatedOrder));
    ...
    let total = 0;
    updatedOrder.forEach(({ ProductId, Count }) => {
  ...
  return (
    <div className="w-full md:w-3/4 bg-white rounded-md shadow" key={order.Order.Id}>
      ...
      {/*<div className="px-6 py-4 font-semibold text-right">Order Total: {formatCurrency(currentOrderTotal)}</div>*/}
    </div>
  );
};
